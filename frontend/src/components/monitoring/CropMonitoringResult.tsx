  ...
  return (
    <div className="space-y-8 pb-10">
      ...
      <motion.div
        ...
        className="relative overflow-hidden p-8 bg-gradient-to-br from-[#63A361] to-[#4a8a4d] rounded-[2.5rem] text-white shadow-2xl shadow-[#63A361]/30"
      >
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center gap-8">
          ...
          <div className="flex flex-col items-center gap-2 px-10 py-8 bg-white/10 backdrop-blur-lg rounded-[2.5rem] border border-white/20 shadow-inner">
            <div className="text-6xl font-black text-[#FFC50F]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <motion.div
              ...
              className={`p-8 bg-white rounded-[2.5rem] border-2 ${getSeverityColor(result.diseaseSeverity).border} shadow-2xl shadow-[#5B532C]/5 relative overflow-hidden group transition-all`}
            >
              ...
              <h3 className="text-xs font-black text-[#5B532C]/30 uppercase tracking-[0.2em] mb-6">Primary Diagnosis</h3>
              <div className={`text-3xl font-black ${getSeverityColor(result.diseaseSeverity).text} mb-3`}>
                {result.diseaseDetected}
              </div>
              <div className="flex items-center gap-3">
                <span className={`px-4 py-1.5 rounded-full text-[11px] font-black uppercase tracking-wider ${getSeverityColor(result.diseaseSeverity).bg} ${getSeverityColor(result.diseaseSeverity).text}`}>
                  {result.diseaseSeverity}
                </span>
                <span className="text-xs font-black text-[#5B532C]/40 uppercase tracking-widest">Active Phase</span>
              </div>
            </motion.div>

            <motion.div
              whileHover={{ y: -8, scale: 1.02 }}
              className={`p-8 bg-white rounded-[2.5rem] border-2 ${getSeverityColor(result.pestSeverity).border} shadow-2xl shadow-[#5B532C]/5 relative overflow-hidden group transition-all`}
            >
              ...
              <h3 className="text-xs font-black text-[#5B532C]/30 uppercase tracking-[0.2em] mb-6">Infestation Assessment</h3>
              <div className={`text-3xl font-black ${getSeverityColor(result.pestSeverity).text} mb-3`}>
                {result.pestInfestation}
              </div>
              <div className="flex items-center gap-3">
                <span className={`px-4 py-1.5 rounded-full text-[11px] font-black uppercase tracking-wider ${getSeverityColor(result.pestSeverity).bg} ${getSeverityColor(result.pestSeverity).text}`}>
          ...
          {/* Charts Card */}
          <div className="p-10 bg-white rounded-[3rem] border border-[#5B532C]/5 shadow-2xl shadow-[#5B532C]/5 relative overflow-hidden">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
              ...
              <div>
                <div className="flex items-center justify-between mb-8">
                  <h3 className="text-xl font-black text-[#5B532C]">Vitality Metrics</h3>
                      ...
                      <Tooltip
                        ...
                        content={({ active, payload }) => {
                          if (active && payload && payload.length) {
                            return (
                              <div className="bg-[#5B532C] text-white p-3 rounded-2xl text-[10px] font-black shadow-xl">
                                {payload[0].value}%
                              </div>
                            );
                          }
                ...
                <div className="grid grid-cols-3 gap-3 mt-4">
                  {healthData.map((h, i) => (
                    <div key={i} className="text-center">
                      <div className="text-lg font-black" style={{ color: h.fill }}>{h.value}%</div>
                      <div className="text-[8px] font-black text-[#5B532C]/40 uppercase">{h.name}</div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-8">
                  <h3 className="text-xl font-black text-[#5B532C]">Tissue Impact</h3>
                  <Target className="w-5 h-5 text-orange-500" />
                </div>
                <div className="h-56 flex items-center justify-center relative">
                  <div className="absolute flex flex-col items-center">
                    <span className="text-3xl font-black text-[#5B532C]">{result.affectedArea}%</span>
                    <span className="text-[8px] font-black text-[#5B532C]/30 uppercase tracking-widest">AFFECTED</span>
                  </div>
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie
          ...
          <div className="p-10 bg-[#FDE7B3]/10 rounded-[3rem] border border-[#5B532C]/10">
            <h3 className="text-xl font-black text-[#5B532C] mb-8 flex items-center gap-4">
              ...
              {result.environmentalFactors.map((f, i) => (
                <div key={i} className="p-6 bg-white rounded-3xl border border-[#5B532C]/5 shadow-xl shadow-[#5B532C]/5 hover:scale-[1.03] transition-transform">
                  <div className="flex items-center justify-between mb-4">
                    <span className="text-xs font-black text-[#5B532C] uppercase tracking-wider">{f.factor}</span>
        ...
        <div className="space-y-8">
          ...
          <div className="p-8 bg-[#5B532C] rounded-[3rem] text-white shadow-2xl shadow-[#5B532C]/30">
            ...
            <div className="space-y-6">
              {result.treatmentRecommendations.slice(0, 4).map((rec, i) => (
                <div key={i} className="flex gap-4 group">
                  <div className="w-10 h-10 rounded-2xl bg-white/10 flex items-center justify-center font-black text-[#FFC50F] shrink-0 group-hover:bg-[#FFC50F] group-hover:text-brown-900 transition-all">
                    0{i + 1}
                  </div>
          ...
          <div className="p-8 bg-white rounded-[3rem] border-2 border-[#63A361]/20 shadow-2xl shadow-gray-100 flex flex-col">
            <div className="flex items-center justify-between mb-10">
              <h3 className="text-lg font-black text-[#5B532C]">Guidance Crawl</h3>
            ...
            <div className="space-y-6 flex-1">
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-[9px] font-black text-red-400 uppercase tracking-widest">Video Tutorial</span>
                      <ExternalLink className="w-3 h-3 text-red-300" />
                    </div>
                    <h4 className="text-sm font-black text-[#5B532C] leading-snug">
              ...
              >
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-[9px] font-black text-[#63A361] uppercase tracking-widest">Article Library</span>
              ...
              <div className="mt-4 p-6 bg-[#FDE7B3]/30 rounded-[2.5rem] border border-[#FFC50F]/20 relative">
                <div className="inline-flex items-center gap-2 px-2 py-0.5 bg-white/50 rounded-full mb-3">
                  ...
                  <span className="text-[8px] font-black text-[#5B532C]/40 uppercase tracking-[0.2em]">Crawl Insight</span>
                </div>
                <p className="text-[11px] text-[#5B532C] font-semibold italic leading-relaxed">
                  ...
                  <span className="text-[#63A361] font-black"> Integrated Pest Management (IPM)</span>. Priority: Remove infected foliage immediately."
                </p>
              </div>
            </div>
          </div>
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            onClick={onRetry}
            className="w-full py-6 bg-white border-2 border-[#5B532C]/10 text-[#5B532C] font-black rounded-[2.5rem] flex items-center justify-center gap-3 shadow-xl shadow-gray-200 transition-all uppercase tracking-[0.2em] text-xs hover:bg-[#5B532C]/5"
          >
