  ...
  return (
    <div className="space-y-8 pb-10">
      ...
      <motion.div
        ...
        className="relative overflow-hidden p-8 bg-gradient-to-br from-[#1b4332] via-[#2d6a4f] to-[#40916c] rounded-[2.5rem] text-white shadow-2xl shadow-[#1b4332]/30"
      >
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center gap-8">
          ...
          <div className="flex flex-col items-center gap-2 px-10 py-8 bg-white/10 backdrop-blur-lg rounded-[2.5rem] border border-white/20 shadow-inner">
            <div className="text-3xl font-black text-green-300 uppercase tracking-tighter">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            <motion.div
              whileHover={{ y: -5 }}
              className="p-8 bg-white rounded-[2.5rem] border border-[#5B532C]/5 shadow-2xl shadow-[#5B532C]/5 flex flex-col items-center text-center"
            >
              <div className="w-12 h-12 bg-green-50 rounded-2xl flex items-center justify-center mb-4">
                <Sprout className="w-6 h-6 text-[#63A361]" />
              </div>
              <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Coverage</span>
              <div className="text-2xl font-black text-[#5B532C]">{result.realTimeMetrics.coveragePercentage}%</div>
              <p className="text-[10px] font-bold text-green-600 mt-1">Excellent Population</p>
            </motion.div>

            <motion.div
              whileHover={{ y: -5 }}
              className={`p-8 bg-white rounded-[2.5rem] border-2 ${getStatusColor(result.weedDensity).border} shadow-2xl shadow-[#5B532C]/5 flex flex-col items-center text-center`}
            >
              ...
              <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Weed Load</span>
              <div className={`text-2xl font-black ${getStatusColor(result.weedDensity).text} uppercase`}>{result.weedDensity}</div>
            ...
            >
              ...
              <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Yield Forecast</span>
              <div className="text-2xl font-black text-[#5B532C] underline decoration-[#63A361] decoration-4 underline-offset-4">{result.yieldPrediction}</div>
            </motion.div>
          </div>
          ...
          <div className="p-10 bg-white rounded-[3rem] border border-[#5B532C]/5 shadow-2xl shadow-[#5B532C]/5">
            <div className="flex items-center justify-between mb-10">
              <h3 className="text-xl font-black text-[#5B532C]">Areal Distribution</h3>
            ...
            <div className="grid grid-cols-3 gap-4 mt-8">
              {coverageData.map((c, i) => (
                <div key={i} className="flex flex-col items-center p-4 bg-gray-50 rounded-2xl border border-gray-100">
                  <span className="text-xl font-black" style={{ color: c.fill }}>{c.value}%</span>
                  <span className="text-[9px] font-black text-gray-400 uppercase tracking-tighter">{c.name} Content</span>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Visible Issues */}
          <div className="p-8 bg-[#FDE7B3]/20 rounded-[2.5rem] border border-[#FFC50F]/30 border-dashed">
            <div className="flex items-start gap-4">
              ...
              <div>
                <h4 className="text-sm font-black text-[#5B532C] mb-1">Anomaly Detection</h4>
        ...
        <div className="space-y-8">
            ...
            <div className="space-y-4">
              {result.interventionPlans.slice(0, 4).map((p, i) => (
                <div key={i} className="flex gap-4 p-4 bg-white/5 rounded-2xl border border-white/5 group hover:bg-white/10 transition-all">
                  <div className="w-8 h-8 rounded-xl bg-green-500 flex items-center justify-center font-black group-hover:scale-110 transition-transform">
                    {i + 1}
                  </div>
          ...
          <div className="p-8 bg-white rounded-[3rem] border border-[#5B532C]/10 shadow-xl shadow-gray-100">
            <div className="flex items-center justify-between mb-8">
              <h3 className="text-lg font-black text-[#5B532C]">Aero Guidance</h3>
              <div className="px-3 py-1 bg-green-100 rounded-full text-[9px] font-black text-green-700 uppercase">Live Crawl</div>
            </div>
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    <h4 className="text-xs font-black text-[#5B532C]">Field Expert</h4>
